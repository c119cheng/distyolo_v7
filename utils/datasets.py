...
class LoadImagesAndLabels(Dataset):  # for training/testing
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        assert l.shape[1] == 6, 'labels require 5+1(distance) columns each'
                        #assert l.shape[1] == 5, 'labels require 5 columns each'
                        assert (l >= 0).all(), 'negative labels'
                        assert (l[:, 1:5] <= 1).all(), 'non-normalized or out of bounds coordinate labels'
                        #assert (l[:, 1:] <= 1).all(), 距離放最後一項 所以不須mormalize
    ...
    def __getitem__(self, index):
        ...
        if nL:
            labels_out[:, 1:] = torch.from_numpy(labels)#現在每一列是(0,class,x,y,w,h,distance)
