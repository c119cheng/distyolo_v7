...
class MyDistLoss(nn.Module): #自定義的距離損失函數
    def __init__(self):
        super(MyDistLoss, self).__init__()
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        lcls, lbox, lobj, ldist = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        #lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox, indices, anchors, tdist = self.build_targets(p, targets)  # targets taget目前是物件等級的標籤 所以我們要把它轉成網格等級的標籤

        # Losses
        #原本計算loss時 prediction的0,1代表xy 2,3代表wh 4代表conf 5以後代表class 現在我們插入距離預測在5 變成讓6以後代表class
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, 6:], self.cn, device=device)  # 因應距離而改動
                    #t = torch.full_like(ps[:, 5:], self.cn, device=device)  # targets
                    t[range(n), tcls[i]] = self.cp
                    #t[t==self.cp] = iou.detach().clamp(0).type(t.dtype)
                    lcls += self.BCEcls(ps[:, 6:], t)  # 因應距離而改動
                ...
                ##distance loss
                self.distloss = MyDistLoss() #根據dist yolo而如此設定
                '''Change by Cheng(tdist to tdist[i])'''
                ldist += self.distloss(ps[:,5], tdist[i])
                #總之可以用類比的方式寫出來吧
        ...
        '''Change by Cheng *=1e-3 to *= 1e-6'''
        ldist *= 1e-6 # according to the cfg in distyolo
    ...
    def build_targets(self, p, targets):
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)  #targgets是一個二維向量 每列有(來自哪個image,class,x...等資訊)
        na, nt = self.na, targets.shape[0]  # number of anchors, targets
        tcls, tbox, indices, anch, tdist = [], [], [], [], []
        #tcls, tbox, indices, anch = [], [], [], []
        
        gain = torch.ones(8, device=targets.device).long()  # normalized to gridspace gain
        #gain = torch.ones(7, device=targets.device).long()  # normalized to gridspace gain
        
        ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt) #(na,nt)大小的矩陣
        targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices #運用repeat方法 讓targets變na個channel 而ai則用None索引讓ai變成(na,nt,1)的張量可以和target合併

        g = 0.5  # bias
        off = torch.tensor([[0, 0],
                            [1, 0], [0, 1], [-1, 0], [0, -1],  # j,k,l,m
                            # [1, 1], [1, -1], [-1, 1], [-1, -1],  # jk,jm,lk,lm
                            ], device=targets.device).float() * g  # offsets 大概是上下左右offset的什麼設定吧

        for i in range(self.nl): #以Channel為單位建立回傳的target
            anchors = self.anchors[i]
            gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain #代表每個layer總共有幾乘幾個grid cell
            
            # Match targets to anchors
            t = targets * gain #targets 經由dataloader返回並且上方cat後  每一列會是(?,class,x(單位grid),y(單位grid),w,h,distance,anchor)
            if nt:
                # Matches
                r = t[:, :, 4:6] / anchors[:, None]  # wh ratio
                j = torch.max(r, 1. / r).max(2)[0] < self.hyp['anchor_t']  # compare
                # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
                t = t[j]  # filter #反正把target壓成1個channel
            ...
            #distance
            '''Change by Cheng(t[:, 7] to t[:, 6])'''
            tdist.append(t[:,6].float())
            
        return tcls, tbox, indices, anch, tdist
...
class ComputeLossOTA:
    ...
    def find_3_positive(self, p, targets):
        ...
        '''參考522行改動'''
        #gain = torch.ones(7, device=targets.device).long()  # normalized to gridspace gain
        gain = torch.ones(8, device=targets.device).long()
